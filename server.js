...
app.get("/api/screener", async (req, res) => {
  try {
    const symbols = [
      "AAPL","TSLA","NVDA","AMD","META","AMZN","GOOGL",
      "MSFT","NFLX","COIN","PLTR","SMCI","AVGO","INTC"
    ];
    ...
    const results = quotes.map(q => {
      ...
      const volumeSpike = volume / avgVol;
      ...
      return {
        ...
        whale: volumeSpike > 2.5 && Math.abs(change) > 2,
        accumulation: volumeSpike > 1.5 && Math.abs(change) < 1.5,
        squeeze: volumeSpike < 1.2 && Math.abs(change) < 1,
        breakout: volumeSpike > 2 && change > 3
      };
    });
